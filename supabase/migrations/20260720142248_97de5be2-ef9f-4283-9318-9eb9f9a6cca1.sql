...
-- Allow owners to insert correction events for their own business only.
CREATE POLICY "Owners can record correction events"
...
GRANT INSERT ON public.transaction_correction_events TO authenticated;

-- Switch correction function to SECURITY INVOKER; RLS on both tables now enforces authorization.
...
BEGIN
  ...
  -- SELECT under invoker RLS: only the owner will see the row.
  ...
  -- UPDATE gated by the "Owners can update their business transactions" policy.
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Correction not permitted';
  END IF;

  -- INSERT gated by the "Owners can record correction events" policy.
