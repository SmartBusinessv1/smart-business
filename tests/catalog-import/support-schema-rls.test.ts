...
import { adminClient, createAnonClient, createTestOwner, type TestOwner } from "../setup/test-clients";
...
describe("catalog_import_batches / catalog_import_rows -- ACL, RLS, and constraints", () => {
  ...
  it("authenticated cannot INSERT/UPDATE/DELETE catalog_import_rows directly (no grant)", async () => {
    ...
    const deleteResult = await ownerA.client.from("catalog_import_rows").delete().eq("batch_id", batchA);
    expect(deleteResult.error).toBeTruthy();
  });

  it("Owner A can SELECT their own batch and rows", async () => {
    const batch = await ownerA.client.from("catalog_import_batches").select("*").eq("id", batchA).maybeSingle();
  ...
  it("Owner B cannot see Owner A's import rows even by batch_id", async () => {
    const result = await ownerB.client.from("catalog_import_rows").select("id").eq("batch_id", batchA);
    expect(result.error).toBeNull();
  ...
  it("an anonymous caller has zero access to either table", async () => {
    ...
    const readBatches = await anon.from("catalog_import_batches").select("id").eq("id", batchA).maybeSingle();
