...
4. **Interpretation — finalization reconciliation.** The Status of row 1.2 and the header Status previously read "AMENDMENT MERGED (PR #606) — MISSION CONTROL ACTIVATION CONFIRMATION PENDING". They now record that Version 1.2 is active from the activation event, the human merge of the finalization pull request. The Status of row 1.1 remains as originally logged (`ACTIVE`), and Version 1.1 is superseded by Version 1.2 at the activation event. The activation date and merge commit are recorded afterward as mandatory factual evidence, and none is recorded here.
