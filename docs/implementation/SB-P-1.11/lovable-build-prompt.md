...
| Authorizing Instruction | `communication/live/instruction1.21.md` |
...
**[MANDATORY]** For Phase 1 (Engineering Contract §16 "Phase 1"):

- Runtime access remains Owner-only. Every catalog action is available only to the authenticated business owner (`businesses.owner_id = auth.uid()`), exactly the pattern already used by SB-P-1.10.
- Every command function must still independently re-check the exact permission flag EIS §8 assigns it — `catalog_view`, `catalog_product_manage`, `catalog_lifecycle_manage`, `catalog_price_manage`, `catalog_tax_manage`, `catalog_cost_manage`, `catalog_inventory_link_manage`, `sale_use` (plus `inventory_view` for linking) — even though only the Owner can currently satisfy that check. This keeps command signatures, authorization boundaries, data structures, and UI gating forward-compatible with Phase 2a so no later signature change is required.
...
- **Exact command surface** (EIS §16) — do not invent alternate names, signatures, or additional commands: `create_catalog_product`, `update_catalog_product_identity`, `update_catalog_product_unit`, `create_catalog_category`, `archive_catalog_category`, `archive_catalog_product`, `reactivate_catalog_product`, `delete_catalog_product`, `record_catalog_selling_price_change`, `schedule_catalog_selling_price`, `cancel_scheduled_catalog_selling_price`, `record_catalog_tax_change`, `update_business_tax_settings`, `record_catalog_reference_cost_change`, `preview_catalog_inventory_link_change`, `assign_or_replace_catalog_inventory_link`, `remove_catalog_inventory_link`, `create_catalog_import_job`, `stage_catalog_import_rows`, `apply_catalog_import_valid_rows`, `create_catalog_pending_action`, `confirm_catalog_pending_action`, `get_catalog_command_outcome`, `catalog_products_search`, `catalog_product_read`, `catalog_products_list_batch`, `list_due_catalog_price_schedule_candidates`, `activate_catalog_price_schedule`.
...
Once a phase is separately authorized and built, the implementing mission must produce, at minimum (Engineering Contract §26, §27; mirroring the evidence structure already established at `docs/implementation/SB-P-1.10/completion-report.md`):

- **Privilege-verification evidence:** exact privilege inspection — not role-existence checks — for all eight command-group owner roles, `catalog_channel_executor`, `catalog_scheduler_executor`, and both service identities (`catalog_channel_service`, `catalog_scheduler_service`); confirmation of every `EXECUTE` grant against its authorized Layer 1 identity; confirmation of every table-level privilege against the EIS §7 grant table; confirmation of `PUBLIC` execute revocation on every command function; confirmed zero direct protected-table DML for both service identities.
- **Scheduler evidence:** confirmation that the Pattern A worker is available and correctly credentialed in the deployed environment, and that `activate_catalog_price_schedule` requires no internal transaction-control statement.
- **Rejection-durability evidence:** every named rejection category confirmed to durably persist its idempotency-key and token-consumption bookkeeping under injected-fault testing.
- **Outcome-scope evidence:** `get_catalog_command_outcome` confirmed to accept no caller-supplied business parameter in its deployed signature.
- **RLS and business-isolation evidence:** confirmed for every new catalog, category, and history table.
- **Test results:** covering every item in EIS §21 "Testing and Verification Matrix" and Engineering Contract §26.
- A **Builder Completion Report** documenting: implementation objective and phase executed; files and components created or modified; Lovable implementation and publish status; Supabase migration status; Supabase RLS verification result; runtime verification result; production-domain verification at `smartbusiness.teamlips.com`; GitHub commit SHA and push status; confirmation that the locked Blueprint, Founder Decision Record, EIS, and Engineering Contract remain unmodified; deviations and their resolution; unresolved defects, if any; final status; recommended next action.
