...
| `supabase/migrations/**` | `COMPLETED — HISTORICAL EVIDENCE, NOT EXECUTABLE` | Twenty-four SQL files at the reviewed repository baseline (`main` at `c3ef55fe0cf94f4491cb2ae257b084f90b49b40b`) record schema/implementation history and reproducibility inputs. This row covers files 1 to 12 in version order (file 11 is annotated separately below), files 13 to 21 are itemized in the rows that follow, and files 22 to 24 are itemized in the `SB-OPS-PROD-SYNC-1.0` rows further below | NO | Never execute from presence; exact files require a new mission |
| `supabase/migrations/20260806120000_sb_p_1_11_impl_1_stage1_schema.sql`, `20260806130000_sb_p_1_11_impl_1_stage2_functions.sql`, `20260808140000_sb_p_1_11_rr_3_tax_lifecycle_rls_remediation.sql` (files 13, 14, 16) | `COMPLETED — HISTORICAL EVIDENCE, NOT EXECUTABLE` | Point-in-time; the current ledger was not re-verified. The `SB-P-1.11` Stage 19 report (`communication/missions/SB-P-1.11/claude-code/19-independent-verification-report.md`, merged 2026-08-17, PR #301) records production `supabase migration list --linked` output "identical for the first 16 migrations". The `SB-P-1.11` GC-39 readiness review (`communication/archive/SB-P-1.11/source/report1.181.md`, 2026-08-28, read-only metadata) records production containing the `SB-P-1.11` migrations through `20260808140000` | NO | Historical evidence only |
| `supabase/migrations/20260808120000_sb_p_1_11_rr_2_category_select_grant.sql` (file 15) | `COMPLETED — HISTORICAL EVIDENCE, NOT EXECUTABLE` | Point-in-time; the current ledger was not re-verified. `SB-P-1.11-RR-2` `report1.67` (`communication/archive/SB-P-1.11/source/report1.67.md`, merged 2026-08-08, PR #155) records `db push` applying it to production. The two ledger observations above also cover it. `report1.181` notes that the remote migration name at this version differs from the test project's recorded name (a metadata divergence; the version is present in both) | NO | Historical evidence only |
| `supabase/migrations/20260810120000_sb_p_1_11_gc_1_catalog_import_support_schema.sql`, `20260811090000_sb_p_1_11_gc_1_security_correction.sql`, `20260819120000_sb_p_1_11_gc_38r_parser_support_schema.sql`, `20260826120000_sb_p_1_11_gc_38r_parser_guard_ambiguity_fix.sql` (files 17 to 20) | `COMPLETED — HISTORICAL EVIDENCE, NOT EXECUTABLE` | Point-in-time; the current ledger was not re-verified. `SB-P-1.11` GC-40, 2026-08-29: each of the four was applied to production in its own authorized step (`report1.185`, `report1.186` and `report1.187` for files 18, 19 and 20, each `PASS`; file 17 was applied and its history entry, first recorded under a generated version `20260829085110`, was reconciled by `GC-40A` in `report1.183` using `migration repair`, without re-running DDL). `report1.182` (2026-08-29) is a final independent read-only package reconciliation: all four versions present and the generated version absent. The record is compiled in `docs/implementation/SB-P-1.11/evidence/gc40-production-migration-reconciliation.md` | NO | Historical evidence only |
| `supabase/migrations/20260724170000_6a0f8a74-e7aa-4200-b54b-3fd57a7c9c62.sql` (file 11, the `SB-P-1.10`-era idempotency fix) | No taxonomy label assigned: its current production status is uncertain, and no label above represents that. Production status: `UNKNOWN` | Two records differ. The 2026-08-17 Stage 19 ledger observation (above) covers the first 16 migrations. The post-completion continuity record (`SB-DOC-1.10-1.11-CONTINUITY-1.0`, 2026-09-03) states that no source reviewed under its audit scope confirms whether this migration is present on current production, and records that as retained verification debt. Neither record is treated as resolving the other | NO | Not represented as applied or awaiting execution. Current status needs a separately authorized read-only ledger verification |
| `supabase/migrations/20260830120000_sb_rel_1_10_1_11_gate2a_c1_inventory_anon_privilege_hardening.sql` (file 21) | No taxonomy label assigned: its current production status is uncertain, and no label above represents that. Production status: `UNVERIFIED` | No primary application, authorization or raw ledger-output record was located; the file's own header describes preparation and test validation. Two **secondary**, documentation-only reports dated 2026-09-13 are retained and attributed, and are not primary production-ledger confirmation: the Supabase Backend Architecture retrospective (`communication/missions/SB-DOC-PHASE1-HISTORY-1.0/room-retrospectives/supabase-backend/01_Retrospective.md`, PR #558, Section 1.6) says the production remote ledger "visibly includes" this version, and the Security & Permissions retrospective (`communication/missions/SB-DOC-PHASE1-HISTORY-1.0/room-retrospectives/security-permissions/01_Retrospective.md`, PR #560) reports a read-only production inspection in which `anon` is no longer a grantee on the three Inventory tables | NO | Not represented as applied or awaiting execution. Current status needs a separately authorized read-only ledger and grant verification |
...
Migration-status rows that carry dates are point-in-time observations attributed to the records named in them. Evidence that a migration was applied is evidence of state and never executable authority, and no row above authorizes any execution. The default-deny execution rule above is unchanged.
...
The 24 files under `supabase/migrations/**` at the reviewed repository baseline (twelve at the time of `SB-GOV-HOUSEKEEPING-1.6`) are version-controlled schema history and possible inputs to a future authorized mission. They are not an executable package, must not be replayed or applied merely because they exist, and were not changed by `SB-GOV-HOUSEKEEPING-1.6`.
