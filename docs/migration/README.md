...
| `SB-OPS-PROD-SYNC-1.0` instr1.6 Phase B (`20260902130000_sb_ops_prod_sync_1_0_instr1_6_phase_b_inventory_item_uniqueness.sql`) | `DEFERRED OR PENDING — NON-EXECUTABLE` | Authored and verified against the isolated test project only; report1.6.md records that the identical statement was proven, in that same isolated environment, to fail against data shaped like production's known duplicate | NO | Blocked ahead of Phase A: requires the Phase C repair (report1.6.md) to be designed, separately authorized, and executed against production first, or this file's own `ADD CONSTRAINT UNIQUE` will fail outright against the current `Mango`/`Milma Milk`/`AVT Tea Powder` duplicate — then still requires its own new, explicit production-execution mission |
