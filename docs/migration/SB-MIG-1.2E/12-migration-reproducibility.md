...
- **Table grants**: production grants all 7 standard privileges (SELECT, INSERT, UPDATE, DELETE, TRUNCATE, REFERENCES, TRIGGER) to `anon`, `authenticated`, and `service_role` on every application table. This traces to `pg_default_acl`: objects created by the `supabase_admin` role receive this broad grant automatically by default — confirmed directly via `pg_default_acl` on production. The exact upstream workflow that originally created production's schema under that role (dashboard Table Editor, Management API, or another Supabase-platform-tooling path) is not established from available evidence. Objects created by the `postgres` role (which is what `supabase db push` runs as, and what owns every table per `pg_tables` — confirmed directly) receive a much narrower default (`TRUNCATE`/`REFERENCES`/`TRIGGER`/`MAINTAIN` only) — explaining exactly why a from-scratch migration replay came up short on functional grants (`GRANT UPDATE, DELETE` on `inventory_movements` specifically, first found in SB-MIG-1.2E-B).
...
- **Platform-specific migration**: `20260723200718`/`20260723200952` grant/revoke a Postgres role named `sandbox_exec`. This role is not persisted as a standing role on either the production or test project — confirmed directly (the `db push` replay fails with "role does not exist" on both). Its exact origin and purpose are not established from available evidence; the naming is suggestive of a platform-managed sandbox/preview execution context, but this is inference, not a confirmed fact. It is safe to skip via migration repair on any fresh replay regardless of its origin, since both migrations are no-ops without the role present.
