...
8. Partial demonstration of a contract records evidence-backed progress and never produces a mature-feature completion upgrade (Section 12).
...
The seven status names are unchanged. Their criteria are:

- `IMPLEMENTED + SUFFICIENTLY ALIGNED` — every applicable Build Now requirement of the contract is demonstrated across the advancing missions, the contract's own completion gate is met, and the required runtime and independent verification exist.
- `IMPLEMENTED BUT INCOMPLETE` — some, but not all, applicable Build Now requirements are demonstrated.
- `IMPLEMENTED BUT MATERIALLY DIVERGENT` — a verified finding shows drift from approved behaviour. Divergence stays visible and is never represented as progress.
- `FOUNDATION EXISTS — FEATURE WORKFLOW MISSING` — a foundation slice is demonstrated and the feature workflow is not.
- `NOT IMPLEMENTED` — no applicable requirement is demonstrated.
- `CANNOT CURRENTLY VERIFY` — the evidence cannot establish the state, and unknown stays unknown.
- `HISTORICAL / PLACEHOLDER ONLY` — unchanged.

A partial mission may move a contract between the existing non-terminal implementation states only where the destination label's criteria are met on evidence (Section 12). It never moves a contract to `IMPLEMENTED + SUFFICIENTLY ALIGNED` unless every criterion of that state is met.
...
The default acceptance state changes only when the contract's completion gate is met and Mission Control accepts it. It does not change while any applicable Build Now requirement remains undemonstrated.

---

# 5. Global Product Completion Register

The **Evidence reference** column holds concise pointers to the accepted mission, to its Feature Coverage and Product Truth Traceability Matrix and Contract Reconciliation (path and row range), and to the verification evidence. This register stays readable at a glance: detailed requirement-level evidence and provenance stay in the FCTM and are never copied here. Until a mission is accepted against a Contract Reconciliation, a row points to the evidence baseline named in the header and says that no Contract Reconciliation exists yet.

| # | Feature / Foundation | Product classification / commercial position | Build commitment | Current implementation | Latest verified advancing SB-P mission | Dependencies remaining | Exact blocker / gap | Allowed now | Not authorized now | Next advancement | Acceptance state | Evidence reference |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 1 | Smart Order & Delivery Assistant | Add-on for Ledger + Manager | **ADD-ON + BUILD NOW — before first 10 pilot merchants** | `NOT IMPLEMENTED` | No dedicated mature-feature mission yet | Permissions; Conversation/WhatsApp; AI; customer identity; Ledger; stock; notifications; delivery proof/location boundaries | No canonical order/delivery workflow or data model | Product Blueprint/discovery once prerequisites and sequencing authorize it | Marketplace expansion; continuous tracking; unconfirmed consequential execution | Dedicated Smart Order & Delivery Product Mission before first 10 pilots | `NOT YET ACCEPTED AS MATURE FEATURE` | Baseline `07_Current_Implementation_Baseline_Refresh_25_Contract_Model.md`; no Contract Reconciliation yet |
...
Rows are updated at Mission Acceptance and their feature-level status is evaluated at Mission Closure (Section 12).
...
- intended delta;
- the mission's FCTM path and baseline, and the rows inherited as residual from earlier missions for each contract advanced;
- dependencies;
- blockers;
...
The mission owns its local scope and evidence. The global register should not be prematurely upgraded because implementation was reported or CI passed. Partial demonstration is recorded as progress and not as completion.

## At Mission Acceptance

Mission Control shall update the affected row(s), in the same change as the acceptance record, only from the accepted Contract Reconciliation and counting only `DEMONSTRATED` and `DEMONSTRATED — CARRIED FORWARD` rows, with:

- latest accepted advancing mission ID;
- new implementation state, only where the destination label's criteria (Section 4.4) are met on evidence;
...
- "Exact blocker / gap", "Dependencies remaining", "Not authorized now" and "Next advancement" are derived from the rows that are not demonstrated and the rows assigned to a later mission, so that no residual disappears into prose.
- A row is never upgraded solely because of a builder report, a merged pull request, green CI, deployment, or mission acceptance, including acceptance with follow-up on a non-demonstrated row.
- **Partial progress.** A partial mission records the fact fields above and may move a contract between the existing non-terminal implementation states only where the destination label's explicit evidence criteria are met. Every movement cites the relevant FCTM row IDs and their status, and every undemonstrated row stays listed. It is a factual update and not mature-feature acceptance, and it is not permission to omit a residual. Unknown stays `CANNOT CURRENTLY VERIFY`.
- The acceptance state stays `NOT YET ACCEPTED AS MATURE FEATURE` while any applicable Build Now obligation or the contract's completion gate remains unmet.
- Neither a movement nor mission acceptance changes a build commitment, a commercial classification or a mission assignment. An actor escalates any proposed removal, deferral, simplification, reclassification or unwarranted completion claim to the Founder through Mission Control.
- A downgrade is recorded when later drift is found.
- Mission Control applies or approves the edit, and the builder never edits this register.

## At Mission Closure

The feature-level status must be evaluated independently from mission-level completion, and the closure confirms the rows updated at acceptance and lists the remaining Build Now requirements per contract.
