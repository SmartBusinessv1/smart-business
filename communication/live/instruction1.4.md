...
### F-4 — Former Owner loses all access upon completed transfer (verbatim)

> another very important thing to consider is if a person transfer a business to another person then that transffered business is no longer have any access to the former person.
> in real world example if anwar transfer his cafe to fathima, anwar can no longer access that cafe details and that is now completely under the identity of fathima.

This is an additional Founder product requirement for the already-discussed dual-approved business transfer, NOT consent to delete Café history or Anwar's identity/other businesses. The former Owner's prior activity remains attributable in preserved audit history, subject to the eventually approved retention/privacy policy, but attribution does not grant continued access.

### F-5 — Former Owner identity deletion cannot harm a transferred business (verbatim)

> before merging #650, I just want to mention the reason I explicitly mention this account transfer detailing is, if anwar once owned this cafe and now transfered so that now this cafe owned by fathima and if anwar now deleting the smart business identity, that doesnt means the cafe will lose its operating status

This is the Founder's explicit cross-business identity-lifecycle invariant. After Anwar validly transfers Café ownership to Fathima, deleting Anwar's personal Smart Business identity must NOT pause, stop, delete, alter the subscription/operating status of, or cascade-delete Fathima's Café or its records. The Café belongs to Fathima; it is not counted as Anwar's remaining owned business and is not subject to Anwar's subsequent identity lifecycle. The prohibition covers both person-initiated account closure and any automatic final-business identity deletion. This does not give Anwar continued Café access. No runtime database proof is claimed; Source 18 T8 ESC-1 must retain the `auth.users ON DELETE CASCADE` issue until designed and independently verified.
...
4. Separates these from **business transfer**, which preserves continuing business data and changes its authorized Owner with two valid approvals; transfer is not equivalent to selling/closing a business workspace or giving away a login. **Additional direct Founder direction (F-4): upon a successfully completed transfer of Anwar's Café to Fathima, Anwar immediately loses ALL Smart Business access to the transferred Café and its details, across dashboard, WhatsApp, in-app conversation, Ask CFO, APIs, files, notifications, exports, pending sessions and delegated paths. Fathima's identity then holds Owner authority over the Café.** No retained Manager/staff membership, stale capability or previous Owner exemption for Anwar is implied; a future independently authorized new access grant would be a distinct event requiring approval by the new Owner. Preserve the former Owner's attributable historical audit entries without granting him viewing rights. Anwar's other businesses and personal identity remain unaffected. Treat completion as an atomic authority handover with runtime authorization revalidation and denial of stale access; exact mechanism, pending transfer races and notifications are subsequent security/engineering design. No claim that the platform can revoke records the former Owner had already lawfully downloaded outside Smart Business.
5. Distinguishes per-business lifecycle from the person's shared identity: **when the final business is permissibly deleted, the Founder directs that the personal identity also be deleted after obligations are satisfied**. This does NOT allow deletion of the human login when other independently owned businesses still exist; flag for separate decision if the person retains memberships in other people's businesses or otherwise still needs a permitted account, rather than silently deleting those permissions or inventing an exception. Deletion of a business must never be an uncontrolled `auth.users` or Owner API cascade. **F-5 invariant: once Café ownership transfers to Fathima, ANY later deletion of Anwar's personal identity must leave Fathima's Café operating, with its subscription, financial/authority history, staff and business records intact, and must not affect Fathima's identity or entitlements.** This is an explicit product-level independence requirement, NOT an assertion that the current `owner_id → auth.users ON DELETE CASCADE` schema already enforces it. Any final-identity deletion design must identify all active/past ownership links and other business memberships and ensure that deleting one person's identity cannot cascade into another person's business; actual owner-history attribution must remain meaningful without retained former-Owner access.
