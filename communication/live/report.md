...
**Applicable CI:** this round modifies `vitest.fast.config.ts`, which is in `full-assurance.yml`'s path filter — a real Full Assurance run is therefore expected and will not be suppressed. Exact-head CI (including Full Assurance) on the corrected, re-pushed commit to be confirmed; not asserted as already complete here.
