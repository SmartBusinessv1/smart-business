...
- **ATTRIBUTION CORRECTION (discovered later in this same evidence-gathering pass):** a targeted research pass found that `SB-P-1.10` — structurally the closest neighboring mission to `SB-P-1.8`/`SB-P-1.9`, using the identical `docs/implementation/` artifact pattern — explicitly credits its own authorship to **"Claude"** and **"Claude Engineering"**, not "Claude Code," in every document byline; "Claude Code" is first explicitly named as "Executing AI" only starting with `SB-P-1.11`'s `communication/missions/SB-P-1.11/claude-code/` stage reports. It is therefore **more likely than not** that `SB-P-1.8` and `SB-P-1.9` were also substantively performed by "Claude" / "Claude Engineering" rather than "Claude Code" specifically, despite this file's original framing. This file is retained under `claude-code/session-evidence/` for its institutional lessons (independent-review-catches-real-defects, transaction-correction pattern, `SECURITY DEFINER` justification discipline), which remain directly relevant to Claude Code's own later, confirmed work (see session-evidence 01 and 05) — but the final synthesizer must treat this era's actor attribution as unresolved, not as confirmed Claude Code history.
