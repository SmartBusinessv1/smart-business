...
| `organizational-learning/tests/context-pack.test.ts`                                                         | new      | 17 focused tests: real-fixture eligibility, deterministic ordering, Candidate 3 preservation, candidate-only exclusion, stale-revision rejection (2 cases), scope exclusion, no-invented-institutionalization (2 cases), supersession (2 cases), determinism (in-process + real CLI process), screening, and CLI argument validation. |
...
Verified independently via both the real CLI (`node organizational-learning/scripts/context-pack.mjs --mission-class operational --system github-actions --system ci --environment ci --related-mission SB-OPS-CI-ARCHITECTURE-1.0`) and the committed test suite. Every eligible item's `resulting_maturity` is exactly `"VALIDATED"`, `promotion_scope` exactly `"MISSION_SCOPED"`, `approving_authority.actor_class` exactly `"mission-control"`, and every one of its evidence references independently resolves `VALID` via the existing, unmodified `validateProvenanceReference`.
...
This round modifies `vitest.fast.config.ts` (adding the new test file to its explicit include list), which is itself named in `full-assurance.yml`'s selective path filter. Per the authorization's explicit instruction ("do not suppress a required workflow"), Full Assurance is therefore expected to actually trigger this time, unlike the candidate/promotion/document-only rounds in Stage 2B/3A. The exact-head result (Lint, Typecheck, Build, Fast Tests, Markdown Quality Gate, and Full Assurance) will be confirmed once the pushed commit's real GitHub Actions runs actually complete, and is not claimed here before that.
