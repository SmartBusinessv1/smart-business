...
**Not opened / explicitly preserved:** Contract 7's Core Stock Capabilities (§2), Supplier Management (§3), Reorder Intelligence (§4), Reorder Authority (§5), Imports and Documents (§6), AI Behaviour (§11), and its own Acceptance Scenarios (§15, 12 scenarios) remain entirely with Contract 7's approved owning mission (Mission Control's review cites `SB-P-1.17` as where Contract 7 remains `BUILD NOW`). This is **not** a `BUILD LATER` reclassification of any part of Contract 7 — it is a scope-boundary clarification only. If Stage 2's own reading of the full contract text finds that SB-P-1.12 must take an obligation from one of these "not opened" sections into scope, that is a T3/T7-type finding for Founder escalation (Source 18 §3.2 item 10), not something this record or Stage 2 alone may decide.
