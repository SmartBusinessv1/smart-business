...
| (c-i) A separately assigned workstream under an already-approved owning mission | An existing mission's own scope is amended to include a distinct Notification-Foundation workstream, without creating a new mission ID | That owning mission's own delivery schedule extends to cover the workstream; no new mission lifecycle needed | The workstream's Verification Checklist sits inside that mission's existing one | A Founder decision naming the specific owning mission and workstream, then a canonical mission-scope/FCTM record — lighter than (c-ii), still not an FCTM row edit alone |
| (c-ii) A genuinely new, separate Product Mission | A new mission ID is authorized specifically for the Notification Foundation | Delays any feature's dependence on the Foundation until that new mission completes its own full Source 18 lifecycle from Stage 1 | That new mission would carry its own independent Verification Checklist | Requires the Founder to authorize creating a new mission (its own Stage 1 Intake Pack) — heavier than (c-i) |

(c-i) and (c-ii) are distinct admissible structures, not the same choice under different names; neither is preferred here over the other or over (a)/(b). Either way, a new assignment needs the Founder's actual decision and a later canonical record — never inferred from this brief.
...
| (c-i) A separately assigned workstream under an already-approved owning mission (e.g. `SB-P-1.18`) | That mission's own scope is amended to include a distinct primitive-construction workstream, without creating a new mission ID | That owning mission's own delivery schedule extends to cover the primitive's construction; no new mission lifecycle needed | The workstream's Verification Checklist sits inside that mission's existing one | A Founder decision naming the specific owning mission and workstream, then a canonical mission-scope/FCTM record — lighter than (c-ii) |
| (c-ii) A genuinely new, separate Product Mission | A new mission ID is authorized specifically for the shared Location primitive | Delays feature dependence on the primitive until that new mission completes its own full Source 18 lifecycle from Stage 1 | That new mission carries its own independent Verification Checklist | Requires Founder authorization of a new mission (its own Stage 1 Intake Pack) — heavier than (c-i) |

(c-i) and (c-ii) are distinct admissible structures, not the same choice under different names; neither is preferred here over the other or over (a)/(b). Either way, a new assignment needs the Founder's actual decision and a later canonical record.
...
| (b) A shared disclosure pattern/template is additionally authored once, centrally | Whoever builds the shared primitive (Question 1) also authors a reusable disclosure pattern/template as an **optional shared foundation mechanism** — this does not replace Contract 22 §16's own requirement that "each feature must define" its 5 disclosures | Front-loads a reusable template into the primitive's own build; does **not** reduce or excuse any consuming feature's own disclosure-defining work | Foundation-level Verification Checklist item for the template's existence **in addition to, not instead of**, each consuming mission's own Verification Checklist item proving its specific feature defined and demonstrated all 5 values (why needed, who sees it, capture point, retention duration, access end) | Founder decision on whether to centralize a template at all, tied to whichever Question 1 answer is chosen — this does not by itself change who is accountable for each feature's 5 disclosures |

**Under either option, the obligation is the same and is not a governance choice:** Contract 22 §16 mandates that every location-consuming feature — `SB-P-1.18`'s attendance/delivery and any later feature alike — itself defines and demonstrates its own 5 disclosure values, evidenced in that feature's own receiving mission's Verification Checklist. A centralized template, if the Founder chooses one, is an available shared mechanism a feature may draw on; it cannot substitute for that feature's own evidence.
