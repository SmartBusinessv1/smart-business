...
11. **What requires Founder or Mission Control decision?** **Mission Control's own determination of Stage 3 triggers T4 and T6** (the residual `anon`-grant Delta finding and the three derived constraints) — corrected 2026-09-22 on Mission Control review to leave these open rather than self-cleared; see Part 4 (`06-stage2-delta-evidence.md` §1). Also the two Contract 22 ambiguous-assignment flags (§12 Notification, §16 Location Foundations, Part 4 §3), routed to Mission Control as a precise unresolved item, not the Founder directly unless Mission Control's review finds otherwise.
