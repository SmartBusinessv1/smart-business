...
4. Surgically reconciled the 15 affected rows in `claude-code/03-stage2-populated-fctm.md` (Correction note 7): 9 Notification rows to `ASSIGNED TO LATER MISSION`/`SB-P-1.15`; 6 Location rows to `ASSIGNED TO LATER MISSION`/`SB-P-1.18` (the 5 per-feature disclosure rows explicitly scoped to `SB-P-1.18`'s own attendance/delivery instance, not asserted as its exclusive future ownership). No row added or removed; `22-§16-2` and `22-§29-9` left unchanged in substance. Updated §G's reconciliation totals and reproducible `grep` commands to the new, actually-verified figures.
