# SB-P-1.12 — Stage 2 Independent Obligation Inventory (CORRECTED — MC-06A, MC-08, MC-09)
...
**Contract 22 total: 4+9+10+5+1+5+9+1+1+1+1+1+4+2+7+1+2+1+10+7+14+3 = 98 pointers** (was 92; +6 for §27). **This now matches the FCTM's actual 98 rows exactly.**
...
| 22 | 98 | 98 | 0 | — |
| 20 | 47 | 47 | 0 | — |
| 17 | 51 | 53 | **+2 (FCTM has more rows)** | two genuine, disclosed 1-item-to-2-row splits: `17-§16` (Stable UI/Testability, +1) and `17-§22` scenario 10 (testable stable identifiers, +1) — the §18/§21 consolidations reported in round 2 no longer exist, now that both sections are represented 1:1 (5 source items → 5 FCTM rows each) |
| 7 | 36 | 36 | 0 | — |
| Build Plan | 18 | 18 | 0 | — |
| **Total** | **357** | **359** | **+2** | entirely attributable to Contract 17's two representation splits above |

**This is the genuine, disclosed set-difference Mission Control's blockers asked for** — two distinct quantities, not a single retroactively-matched total. The only contract with a real difference (Contract 17) has that difference traced to two specific, named rows and explained as a representation choice, not asserted away. This round's corrections (MC-09A/B/C) closed the round-2 consolidations at §18/§21 and the round-2 undercounts at §8/§27, leaving a materially tighter, fully 1:1 picture everywhere except those two genuine splits.
...
- **Contract 22 (0 net, but was 0-with-a-hidden-undercount before MC-09C):** §27's 7-item mandatory disclosure checklist was left as a single row on the mistaken premise that it matched Contract 21 §7–§9's single-rule pattern; Mission Control correctly distinguished mandatory individually-required items from alternative means to one end. §27 is now 7 source pointers mapped to 7 FCTM rows. **Resolution: a genuine FCTM gap, now closed — this section had simply never been examined for the mixed-checklist pattern before this round.**
...
- **Contract 17 (+2, genuine and explained, tighter than round 2's −4):** traces to exactly two rows: `17-§16` (1 source item → 2 FCTM rows, consuming-mission split, +1) and `17-§22` scenario 10 (1 source item → 2 FCTM rows, consuming-mission split, +1). §18 and §21, which round 2 had explained as consolidations (5 source items → 2 FCTM rows each), are corrected this round to genuine 1:1 representation (5 source items → 5 FCTM rows each, per MC-09A) — that consolidation no longer exists because the FCTM itself was fixed, not just this inventory's count of it. **Resolution: this is the one contract where source-obligation count and FCTM representation-row count genuinely differ, for a disclosed, auditable reason (two sections whose content legitimately splits across missions) — not a missing obligation, and not asserted equal to avoid explaining it.**
- **Contract 7 (0 net, but was +1-with-a-structural-error before MC-09B):** round 2 added a 3rd §8 pointer ("supplier payment state") but kept the section framed as 3 touched/`IN SCOPE` items, including one (`Catalog↔Inventory↔Transactions separation preserved`) that did not correspond to any real §8 source item — §8 was never in MC-03's actual touched-scope list. This round: §8 rebuilt from source as its true 5 items (1 integrated rule + 4 examples), all `ASSIGNED TO LATER MISSION`; the genuine architecture-preservation concern this mission does own remains correctly tracked at `BP-§7-3`/`BP-§7-4`. **Resolution: an unauthorized-scope + undercount error, now closed — not a legitimate split.**
...
**Total source pointers: 107+98+47+51+36+18 = 357.** **Total FCTM rows: 107+98+47+53+36+18 = 359** (`03-stage2-populated-fctm.md` §G, `grep`-counted). These are reported as two distinct figures per Mission Control's explicit instruction, not blended into one number.
