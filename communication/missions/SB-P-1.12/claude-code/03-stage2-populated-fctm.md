# SB-P-1.12 — Stage 2 Populated FCTM (CANONICAL, with a DRAFT MC-16 15-row reconciliation pending human merge — see Correction Note 7)

**Status:** `CANONICAL (PR #624, MC-12) — 15-ROW SURGICAL RECONCILIATION DRAFT PENDING MISSION CONTROL REVIEW AND FOUNDER HUMAN MERGE (MC-16)`. Source 18 §3.2/§6 Stages 2–3. The 358 non-escalated rows are unaffected and remain canonical; only the 15 rows named in Correction note 7 change in this draft, and only upon this pull request's human merge.
...
**Disposition vocabulary:** `IN SCOPE`, `ASSIGNED TO LATER MISSION` (names the mission and citation), `DELEGATED`, `NOT APPLICABLE`, `ESCALATED` (Source 18 §3.2 item 10, "a conflict or proposed change awaiting the Founder"; see `06-stage2-delta-evidence.md` Part 4 §3 for the canonical Stage 2 reasoning that first applied this value to 15 Contract 22 §12/§16 items). Per Source 18 §3.2 item 3, `UNRESOLVED FOUNDER DECISION` means "on the approved unresolved list" — reserved for items Build Plan §15 already names, not used by any row. **Correction note 7 (2026-09-23, MC-16, DRAFT):** the 15 rows that carried `ESCALATED` through the canonical Stage 2 record now carry `ASSIGNED TO LATER MISSION` in this draft round, reflecting the Founder's confirmed 2026-09-23 dialogue transcribed in `founder/03-stage3-founder-product-decision-record.md` — see Correction note 7 above and §G below for the full reconciliation. This file's row-level disposition is not itself the canonical Stage 3 gate closure; that requires this pull request's Mission Control review and Founder human merge.
...
**Correction note 7 (2026-09-23, MC-16 — surgical 15-row reconciliation, DRAFT pending Mission Control review and Founder human merge).** Following the Founder Brief (PR #626, canonical) and a Founder dialogue Mission Control recorded in `mission-control/05-stage3-decision-record-preparation-authorization.md` (MC-16), Founder Riyas PK confirmed mission assignments for all 15 `ESCALATED` rows, transcribed in `founder/03-stage3-founder-product-decision-record.md`. This round surgically reconciles only those 15 rows: `22-§12-1` through `22-§12-9` move from `ESCALATED` to `ASSIGNED TO LATER MISSION` (`SB-P-1.15`, Founder Decision `FPDR-1`); `22-§16-1` moves from `ESCALATED` to `ASSIGNED TO LATER MISSION` (`SB-P-1.18`, Founder Decision `FPDR-2`); `22-§16-3` through `22-§16-7` move from `ESCALATED` to `ASSIGNED TO LATER MISSION` (`SB-P-1.18`, Founder Decision `FPDR-3`), each citation explicitly scoped to `SB-P-1.18`'s own attendance/delivery feature and explicitly **not** asserting `SB-P-1.18` as the exclusive owner of every future location-consuming feature's disclosures — see `claude-code/12-stage3-gate-evidence-and-row-reconciliation.md` for the general-obligation scope note this single-mission "Assigned mission" cell cannot itself fully carry. `22-§16-2` (`IN SCOPE`) and `22-§29-9` (`ASSIGNED TO LATER MISSION`, `SB-P-1.18`) are **unchanged**, per the Founder's own confirmation not to reopen them. No row added or removed; no source pointer, build commitment or commercial classification changed; every other row on this file is untouched. **This reconciliation is DRAFT — it is not canonical until this pull request is reviewed by Mission Control and human-merged; until then the historically-`ESCALATED` status remains the last canonical record per `mission-control/05-stage3-decision-record-preparation-authorization.md` §1.**

**Correction note 7 addendum (2026-09-23, MC-17 — same-PR narrow correction, DRAFT).** Mission Control's review of PR #628 ([comment `5794742283`](https://github.com/SmartBusinessv1/smart-business/pull/628#issuecomment-5794742283)) found the first draft of this reconciliation had, despite the "no source pointer... changed" claim above, actually rewritten the 15 rows' Source pointer cells (adding "corrected 2026-09-23, MC-16 DRAFT" prose and, on `22-§12-1`/`22-§16-1`, deleting the original MC-11 provenance text) and had added an unauthorized confirmation sentence to the untouched `22-§16-2` row. **MC-17A/B, corrected:** all 15 rows' Source pointer cells are now byte-identical to canonical #624/#627; `22-§16-2` is now byte-identical to canonical. **MC-17C, corrected:** `22-§12-2`'s evidence cell now states only that `FPDR-1` resolves Notification-Foundation *ownership*, not that the Human-Language-Foundation reuse question is itself resolved. **MC-17D, corrected:** each of the 5 disclosure rows (`22-§16-3`–`-7`), not only the first, now states in its own evidence cell that `SB-P-1.18` supplies that value for attendance/delivery only and that every future consuming feature's own mission independently defines and verifies its own value. §G's disposition-counting methodology is also corrected below to parse the actual Disposition column rather than search whole rows for a keyword.
...
| 22-§12-1 | §12 Notification Foundation — recipient identity/role (corrected 2026-09-22, MC-11: relabeled from `UNRESOLVED FOUNDER DECISION` — Source 18 §3.2 item 3 reserves that value for items already on Build Plan §15's approved unresolved list, which does not name Notification/Location foundation ownership; the correct treatment for a newly-discovered, unassigned obligation is `ESCALATED` per §3.2 item 10) | BUILD NOW | CORE-ARCH | `SB-P-1.15` | ASSIGNED TO LATER MISSION | **DRAFT (MC-16, Correction note 7) — Founder Decision `FPDR-1`: distinct Shared Notification Foundation workstream within existing `SB-P-1.15`; cross-mission/feature/channel reuse required; `SB-P-1.20` WhatsApp must reuse it, not duplicate its logic** | `founder/03-stage3-founder-product-decision-record.md` |
| 22-§12-2 | §12 — language | BUILD NOW | CORE-ARCH | `SB-P-1.15` | ASSIGNED TO LATER MISSION | **DRAFT (MC-16, Correction note 7) — Founder Decision `FPDR-1`, same basis as `22-§12-1`. This resolves notification-specific *ownership* only: `SB-P-1.15` becomes the Notification Foundation's owning mission. It does not itself implement or verify reuse of the existing Contract 22 §8 Human Language Foundation (`SB-P-1.13`) — `SB-P-1.15` remains responsible for its own future design/verification of appropriate integration with `SB-P-1.13`, not asserted as already resolved by this row** | `founder/03-stage3-founder-product-decision-record.md` |
| 22-§12-3 | §12 — channel preference/availability | BUILD NOW | CORE-ARCH | `SB-P-1.15` | ASSIGNED TO LATER MISSION | **DRAFT (MC-16, Correction note 7) — Founder Decision `FPDR-1`, same basis as `22-§12-1`** | `founder/03-stage3-founder-product-decision-record.md` |
| 22-§12-4 | §12 — template/provider requirements | BUILD NOW | CORE-ARCH | `SB-P-1.15` | ASSIGNED TO LATER MISSION | **DRAFT (MC-16, Correction note 7) — Founder Decision `FPDR-1`, same basis as `22-§12-1`** | `founder/03-stage3-founder-product-decision-record.md` |
| 22-§12-5 | §12 — delivery/retry state | BUILD NOW | CORE-ARCH | `SB-P-1.15` | ASSIGNED TO LATER MISSION | **DRAFT (MC-16, Correction note 7) — Founder Decision `FPDR-1`, same basis as `22-§12-1`** | `founder/03-stage3-founder-product-decision-record.md` |
| 22-§12-6 | §12 — duplicate suppression | BUILD NOW | CORE-ARCH | `SB-P-1.15` | ASSIGNED TO LATER MISSION | **DRAFT (MC-16, Correction note 7) — Founder Decision `FPDR-1`, same basis as `22-§12-1`** | `founder/03-stage3-founder-product-decision-record.md` |
| 22-§12-7 | §12 — privacy | BUILD NOW | CORE-ARCH | `SB-P-1.15` | ASSIGNED TO LATER MISSION | **DRAFT (MC-16, Correction note 7) — Founder Decision `FPDR-1`, same basis as `22-§12-1`** | `founder/03-stage3-founder-product-decision-record.md` |
| 22-§12-8 | §12 — link to originating business event | BUILD NOW | CORE-ARCH | `SB-P-1.15` | ASSIGNED TO LATER MISSION | **DRAFT (MC-16, Correction note 7) — Founder Decision `FPDR-1`, same basis as `22-§12-1`** | `founder/03-stage3-founder-product-decision-record.md` |
| 22-§12-9 | §12 — notification delivery separate from business-event completion | BUILD NOW | CORE-ARCH | `SB-P-1.15` | ASSIGNED TO LATER MISSION | **DRAFT (MC-16, Correction note 7) — Founder Decision `FPDR-1`, same basis as `22-§12-1`** | `founder/03-stage3-founder-product-decision-record.md` |
...
| 22-§16-1 | §16 Location Foundation — shared purpose-limited primitive (use one primitive rather than independent tracking systems) (corrected 2026-09-22, MC-11: relabeled from `UNRESOLVED FOUNDER DECISION` — not on Build Plan §15's approved unresolved list; the correct Source 18 §3.2 item 10 treatment for a newly-discovered, unassigned obligation is `ESCALATED`) | BUILD NOW | CORE-ARCH | `SB-P-1.18` | ASSIGNED TO LATER MISSION | **DRAFT (MC-16, Correction note 7) — Founder Decision `FPDR-2`: distinct, reusable, purpose-limited Shared Location Foundation workstream within existing `SB-P-1.18`; no new mission ID; `SB-P-1.12` retains its own already-approved authority/permission/isolation rules that the primitive consumes** | `founder/03-stage3-founder-product-decision-record.md` |
...
| 22-§16-3 | §16 — each feature must define why location is needed | BUILD NOW | CORE-ARCH | `SB-P-1.18` (named instance: attendance/delivery only) | ASSIGNED TO LATER MISSION | **DRAFT (MC-16, Correction note 7) — Founder Decision `FPDR-3`: the owning mission of *each* location-consuming feature must independently define and demonstrate all five §16-3–§16-7 disclosures; `SB-P-1.18` supplies this one for attendance/delivery specifically. This is a standing, recurring obligation binding whichever mission later builds any additional location-consuming feature — `SB-P-1.18` does not carry, and is not asserted as, the verification owner for any future feature beyond attendance/delivery** | `founder/03-stage3-founder-product-decision-record.md`; `12-stage3-gate-evidence-and-row-reconciliation.md` |
| 22-§16-4 | §16 — each feature must define who may see it | BUILD NOW | CORE-ARCH | `SB-P-1.18` (named instance: attendance/delivery only) | ASSIGNED TO LATER MISSION | **DRAFT (MC-16, Correction note 7) — Founder Decision `FPDR-3`, same basis as `22-§16-3`: `SB-P-1.18` supplies this for attendance/delivery only; every future location-consuming feature's own mission independently defines and verifies its own value, not `SB-P-1.18`** | `founder/03-stage3-founder-product-decision-record.md`; `12-stage3-gate-evidence-and-row-reconciliation.md` |
| 22-§16-5 | §16 — each feature must define when it is captured | BUILD NOW | CORE-ARCH | `SB-P-1.18` (named instance: attendance/delivery only) | ASSIGNED TO LATER MISSION | **DRAFT (MC-16, Correction note 7) — Founder Decision `FPDR-3`, same basis as `22-§16-3`: `SB-P-1.18` supplies this for attendance/delivery only; every future location-consuming feature's own mission independently defines and verifies its own value, not `SB-P-1.18`** | `founder/03-stage3-founder-product-decision-record.md`; `12-stage3-gate-evidence-and-row-reconciliation.md` |
| 22-§16-6 | §16 — each feature must define how long it is retained | BUILD NOW | CORE-ARCH | `SB-P-1.18` (named instance: attendance/delivery only) | ASSIGNED TO LATER MISSION | **DRAFT (MC-16, Correction note 7) — Founder Decision `FPDR-3`, same basis as `22-§16-3`: `SB-P-1.18` supplies this for attendance/delivery only; every future location-consuming feature's own mission independently defines and verifies its own value, not `SB-P-1.18`** | `founder/03-stage3-founder-product-decision-record.md`; `12-stage3-gate-evidence-and-row-reconciliation.md` |
| 22-§16-7 | §16 — each feature must define when access ends | BUILD NOW | CORE-ARCH | `SB-P-1.18` (named instance: attendance/delivery only) | ASSIGNED TO LATER MISSION | **DRAFT (MC-16, Correction note 7) — Founder Decision `FPDR-3`, same basis as `22-§16-3`: `SB-P-1.18` supplies this for attendance/delivery only; every future location-consuming feature's own mission independently defines and verifies its own value, not `SB-P-1.18`** | `founder/03-stage3-founder-product-decision-record.md`; `12-stage3-gate-evidence-and-row-reconciliation.md` |
...
**Re-verified 2026-09-22 (MC-06 fifth re-review correction cycle, sixth correction round — MC-11).** Mission Control's fifth re-review ([comment `5779330365`](https://github.com/SmartBusinessv1/smart-business/pull/624#issuecomment-5779330365)) accepted MC-10's obligation enumeration but found the 15 rows' `UNRESOLVED FOUNDER DECISION` disposition itself was not Source 18 vocabulary-compliant — that value means "on the approved unresolved list" (Build Plan §15's nine named topics), and Notification/Location foundation ownership is not one of them (independently re-verified: neither term appears anywhere in the Build Plan document). All 15 relabeled to `ESCALATED` ("a conflict or proposed change awaiting the Founder," §3.2 item 10) — no row count change, no source-pointer change, no reassignment to an existing mission (none was found on recheck). **This round's totals (373; 228/98/2/30/15) are the last canonically-merged figures (PR #624, MC-12) and remain historically accurate; see the MC-16 round below for the current DRAFT state.**

**Re-verified 2026-09-23 (MC-16, seventh correction round — DRAFT, pending Mission Control review and Founder human merge).** Following the Founder's confirmed 2026-09-23 dialogue (`mission-control/05-stage3-decision-record-preparation-authorization.md`, `founder/03-stage3-founder-product-decision-record.md`), all 15 `ESCALATED` rows are surgically reconciled to `ASSIGNED TO LATER MISSION`: the 9 Notification rows (`22-§12-1`–`-9`) to `SB-P-1.15`; the 6 Location rows (`22-§16-1`, `22-§16-3`–`-7`) to `SB-P-1.18`. No row added or removed; no build commitment or commercial classification changed. **Every reconciled row's Source pointer cell is byte-identical to the canonical #624/#627 text** — Founder Decision IDs, assignment consequences and draft markers live only in the Assigned-mission/Disposition/Evidence cells and in this correction note, per Mission Control's MC-17A finding. `22-§16-2` is likewise byte-identical to canonical (MC-17B); `22-§29-9` is untouched.

**Re-verified 2026-09-23 (MC-16, eighth correction round — MC-17, DRAFT, pending Mission Control review and Founder human merge).** Mission Control's review ([PR #628 comment `5794742283`](https://github.com/SmartBusinessv1/smart-business/pull/628#issuecomment-5794742283)) found the prior round had rewritten the 15 rows' Source-pointer cells (deleting the original MC-11 provenance prose on `22-§12-1`/`22-§16-1`) and had added an unauthorized confirmation sentence to the untouched `22-§16-2` row (**MC-17A/MC-17B**, both corrected above); had let `22-§12-2`'s evidence cell overstate that the Notification/Human-Language-Foundation overlap was "resolved" by the ownership assignment, when only ownership is resolved and `SB-P-1.15` still owns its own future reuse design/verification (**MC-17C**, corrected above); and asked that the standing "every future feature independently verifies its own five values, `SB-P-1.18` does not carry future missions' verification" rule be restated in each of the five disclosure rows' own evidence cell, not only the first (**MC-17D**, corrected above). Mission Control also found the prior round's disposition-counting method — a whole-row `grep "ESCALATED"` — was unsound once source pointers legitimately contain that word as historical citation text (as they now do again, restored per MC-17A), and required counting the actual Disposition column instead. **The reproducible commands below are corrected accordingly: each disposition count parses field 7 (the Disposition column) of this file's pipe-delimited rows, via `awk -F'|'`, not a whole-row keyword `grep`.**

Reproducible commands re-run against this file at its current head (exact SHA reported in the accompanying gate-evidence file). Field 7 is the Disposition column: `| Row ID | Source pointer | Build commitment | Commercial classification | Assigned mission | Disposition | Citation / evidence |` — the leading empty split (before the row's opening `|`) is field 1, so `awk -F'|'` numbers Row ID as `$2` through Citation as `$8`, and Disposition is `$7`.
...
grep -cE "^\| (21|22|20|17|7|BP)-§" 03-stage2-populated-fctm.md                                               # 373 (total, unchanged)
grep -E "^\| (21|22|20|17|7|BP)-§" 03-stage2-populated-fctm.md | awk -F'|' '$7 ~ /IN SCOPE/'                | wc -l   # 228 (unchanged)
grep -E "^\| (21|22|20|17|7|BP)-§" 03-stage2-populated-fctm.md | awk -F'|' '$7 ~ /ASSIGNED TO LATER MISSION/' | wc -l # 113 (was 98, +15 this round)
grep -E "^\| (21|22|20|17|7|BP)-§" 03-stage2-populated-fctm.md | awk -F'|' '$7 ~ /DELEGATED/'                | wc -l   # 2 (unchanged)
grep -E "^\| (21|22|20|17|7|BP)-§" 03-stage2-populated-fctm.md | awk -F'|' '$7 ~ /NOT APPLICABLE/'            | wc -l   # 30 (unchanged)
grep -E "^\| (21|22|20|17|7|BP)-§" 03-stage2-populated-fctm.md | awk -F'|' '$7 ~ /ESCALATED/'                 | wc -l   # 0 (was 15, all relabeled)
grep -E "^\| (21|22|20|17|7|BP)-§" 03-stage2-populated-fctm.md | awk -F'|' '$8 ~ /^ — $/'                     | wc -l   # 0 (empty citation gaps, field 8)
grep -E "^\| (21|22|20|17|7|BP)-§" 03-stage2-populated-fctm.md | awk -F'|' '$7 ~ /IN SCOPE/ && $7 ~ /ASSIGNED TO LATER MISSION/' | wc -l  # 0 (no double-counted row)
```

| Contract | Rows | Change this cycle |
|---|---|---|
| 21 | 107 | 0 |
| 22 | 112 | 0 rows added/removed — 15 rows' Disposition/Assigned-mission cells changed `ESCALATED`/`PENDING FOUNDER ASSIGNMENT` → `ASSIGNED TO LATER MISSION`/`SB-P-1.15` or `SB-P-1.18` (DRAFT, MC-16, corrected MC-17); Source pointer cells restored byte-identical to canonical |
| 20 | 47 | 0 |
| 17 | 53 | 0 |
| 7 (limited) | 36 | 0 |
| Build Plan §10.1 + §7 | 18 | 0 |
| **Total** | **373** | **0 (disposition/assigned-mission relabel only; source text unchanged)** |

**Disposition totals (current DRAFT, MC-16/MC-17, pending Mission Control review and Founder human merge, counted from the actual Disposition column, field 7): `IN SCOPE` 228 + `ASSIGNED TO LATER MISSION` 113 + `DELEGATED` 2 + `NOT APPLICABLE` 30 + `ESCALATED` 0 = 373.** Verified no row double-counts across any pair of dispositions in field 7. `UNRESOLVED FOUNDER DECISION`: 0 rows. **Last canonically-merged totals (PR #624, MC-12): `IN SCOPE` 228, `ASSIGNED TO LATER MISSION` 98, `DELEGATED` 2, `NOT APPLICABLE` 30, `ESCALATED` 15 — these remain the canonical record of Product Truth coverage until this pull request is reviewed and human-merged.**

**Citation-gap count: 0 empty cells (field 8), 0 bare-implementation-tag-only cells** (unchanged; all 15 reconciled rows carry real, checkable citations to the Founder Product Decision Record).

This total (373) is **not** a target figure — this round changes no row count, only 15 rows' Disposition and Assigned-mission values, per the Founder's confirmed decision. Source pointer cells for all 373 rows, including the 15 reconciled ones, are unchanged from canonical #624/#627.
...
**No row carries `UNRESOLVED FOUNDER DECISION`.** Historically (canonical Stage 2, PR #624/MC-12), 15 rows (`22-§12-1`–`-9`, `22-§16-1`, `22-§16-3`–`-7`) carried `ESCALATED` — see `06-stage2-delta-evidence.md` Part 4 §3 for that canonical reasoning, still the last canonically-merged record. **Correction note 7 (2026-09-23, MC-16, DRAFT):** in this draft reconciliation round, all 15 now carry `ASSIGNED TO LATER MISSION` (`SB-P-1.15` for the 9 Notification rows, `SB-P-1.18` for the 6 Location rows), reflecting the Founder's confirmed 2026-09-23 dialogue transcribed in `founder/03-stage3-founder-product-decision-record.md` — this is **not** canonical until this pull request is reviewed and human-merged. `22-§16-2` and `22-§29-9` remain unchanged, per the Founder's own instruction not to reopen them. **The mission-level Stage 3 trigger determination is not concluded by this document or by Claude Code** — T7 was genuinely triggered by the historical 15 `ESCALATED` rows and T4 independently by the `PRODUCT-AFFECTING` residual-`anon` Delta finding (`06-stage2-delta-evidence.md` Part 4 §1); both are addressed in the Founder Product Decision Record and gate-evidence files, not resolved by a row-level disposition change alone.
