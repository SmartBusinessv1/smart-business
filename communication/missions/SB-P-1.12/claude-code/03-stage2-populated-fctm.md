...
| 22-§29-9 | Scenario 9: purpose-limited location | BUILD NOW | CORE-ARCH | SB-P-1.12 (default; unresolved) | **IN SCOPE by fail-closed default — corrected 2026-09-22 (MC-06B): this row's disposition previously contradicted its own assigned-mission cell.** Follows 22-§16's unresolved-assignment status exactly (same section); no later-mission naming source exists for this row independent of 22-§16, and 22-§16 itself lacks one | `06-stage2-delta-evidence.md` Part 4 §3; same basis as `22-§16` |
...
| 20-§22-1 | Explicit Non-goals — "staff setup never default-grants Owner intelligence" (duplicate of 20-§16-3) | BUILD NOW | ACT | SB-P-1.12 | IN SCOPE — negative/must-not-appear check | Contract 20 §22, restates §16's own rule |
| 20-§22-2 | Explicit Non-goals — remaining onboarding non-goals (old routes, manipulative tactics, hardcoded trial assumption, forced full setup) | BUILD NOW | ACT | `SB-P-1.19` | ASSIGNED TO LATER MISSION | `L-A20` |
...
**Re-verified 2026-09-22 (MC-06 correction cycle)** after fixing 97 citation gaps and one cross-column disposition contradiction (`22-§29-9`, corrected from `ASSIGNED TO LATER MISSION` to `IN SCOPE` — see its row for the reasoning). Total row count is unchanged (337, no row added or removed); the `IN SCOPE`/`ASSIGNED TO LATER MISSION` split shifted by exactly one row as a direct, disclosed result of that correction.
...
grep -E "^\| (21|22|20|17|7|BP)-§" 03-stage2-populated-fctm.md | grep -c "IN SCOPE"                    # 223
grep -E "^\| (21|22|20|17|7|BP)-§" 03-stage2-populated-fctm.md | grep -c "ASSIGNED TO LATER MISSION"   # 82
...
grep -E "^\| (21|22|20|17|7|BP)-§" 03-stage2-populated-fctm.md | grep -cE '\| — \|$'                     # 0 (citation gaps)
```
...
**Disposition totals: `IN SCOPE` 223 + `ASSIGNED TO LATER MISSION` 82 + `DELEGATED` 2 + `NOT APPLICABLE` 30 = 337.** Verified no row double-counts across dispositions (checked explicitly: zero rows match both `IN SCOPE` and `ASSIGNED TO LATER MISSION` in the same cell — this check itself caught a bug during this correction cycle, where the fix text for `22-§29-9` accidentally re-introduced the literal phrase "ASSIGNED TO LATER MISSION" inside a negation; corrected and re-verified at zero overlap).

**Citation-gap count: 0** (was 97 before this correction cycle; every row now carries a disposition-supporting citation — a specific contract section/item reference, an exact Build Plan mission-assignment citation, a legend ID for a recurring basis, or a cross-reference to the Delta document for the two fail-closed-default rows).
...
**Reconciliation against the independent obligation inventory:** see `04-stage2-obligation-inventory.md` §3 for the genuinely source-first obligation pointer inventory and its explicit set-difference check against this file's actual row IDs.
