...
| 22-§12-1 | §12 Notification Foundation — recipient identity/role (corrected 2026-09-23, MC-16 DRAFT: reconciled to the Founder-confirmed assignment — see Correction note 7) | BUILD NOW | CORE-ARCH | `SB-P-1.15` | ASSIGNED TO LATER MISSION | **DRAFT — Founder Decision `FPDR-1`: distinct Shared Notification Foundation workstream within existing `SB-P-1.15`; cross-mission/feature/channel reuse required; `SB-P-1.20` WhatsApp must reuse it, not duplicate its logic** | `founder/03-stage3-founder-product-decision-record.md` |
| 22-§12-2 | §12 — language (corrected 2026-09-23, MC-16 DRAFT) | BUILD NOW | CORE-ARCH | `SB-P-1.15` | ASSIGNED TO LATER MISSION | **DRAFT — Founder Decision `FPDR-1`, same basis as `22-§12-1`; overlap with Contract 22 §8 Human Language Foundation (`SB-P-1.13`) resolved by the Founder's own choice of `SB-P-1.15` as the Notification Foundation owner** | `founder/03-stage3-founder-product-decision-record.md` |
| 22-§12-3 | §12 — channel preference/availability (corrected 2026-09-23, MC-16 DRAFT) | BUILD NOW | CORE-ARCH | `SB-P-1.15` | ASSIGNED TO LATER MISSION | **DRAFT — Founder Decision `FPDR-1`, same basis as `22-§12-1`** | `founder/03-stage3-founder-product-decision-record.md` |
| 22-§12-4 | §12 — template/provider requirements (corrected 2026-09-23, MC-16 DRAFT) | BUILD NOW | CORE-ARCH | `SB-P-1.15` | ASSIGNED TO LATER MISSION | **DRAFT — Founder Decision `FPDR-1`, same basis as `22-§12-1`** | `founder/03-stage3-founder-product-decision-record.md` |
| 22-§12-5 | §12 — delivery/retry state (corrected 2026-09-23, MC-16 DRAFT) | BUILD NOW | CORE-ARCH | `SB-P-1.15` | ASSIGNED TO LATER MISSION | **DRAFT — Founder Decision `FPDR-1`, same basis as `22-§12-1`** | `founder/03-stage3-founder-product-decision-record.md` |
| 22-§12-6 | §12 — duplicate suppression (corrected 2026-09-23, MC-16 DRAFT) | BUILD NOW | CORE-ARCH | `SB-P-1.15` | ASSIGNED TO LATER MISSION | **DRAFT — Founder Decision `FPDR-1`, same basis as `22-§12-1`** | `founder/03-stage3-founder-product-decision-record.md` |
| 22-§12-7 | §12 — privacy (corrected 2026-09-23, MC-16 DRAFT) | BUILD NOW | CORE-ARCH | `SB-P-1.15` | ASSIGNED TO LATER MISSION | **DRAFT — Founder Decision `FPDR-1`, same basis as `22-§12-1`** | `founder/03-stage3-founder-product-decision-record.md` |
| 22-§12-8 | §12 — link to originating business event (corrected 2026-09-23, MC-16 DRAFT) | BUILD NOW | CORE-ARCH | `SB-P-1.15` | ASSIGNED TO LATER MISSION | **DRAFT — Founder Decision `FPDR-1`, same basis as `22-§12-1`** | `founder/03-stage3-founder-product-decision-record.md` |
| 22-§12-9 | §12 — notification delivery separate from business-event completion (corrected 2026-09-23, MC-16 DRAFT) | BUILD NOW | CORE-ARCH | `SB-P-1.15` | ASSIGNED TO LATER MISSION | **DRAFT — Founder Decision `FPDR-1`, same basis as `22-§12-1`** | `founder/03-stage3-founder-product-decision-record.md` |
...
| 22-§16-1 | §16 Location Foundation — shared purpose-limited primitive (use one primitive rather than independent tracking systems) (corrected 2026-09-23, MC-16 DRAFT: reconciled to the Founder-confirmed assignment — see Correction note 7) | BUILD NOW | CORE-ARCH | `SB-P-1.18` | ASSIGNED TO LATER MISSION | **DRAFT — Founder Decision `FPDR-2`: distinct, reusable, purpose-limited Shared Location Foundation workstream within existing `SB-P-1.18`; no new mission ID; `SB-P-1.12` retains its own already-approved authority/permission/isolation rules that the primitive consumes** | `founder/03-stage3-founder-product-decision-record.md` |
| 22-§16-2 | §16 — default continuous employee surveillance is rejected | BUILD NOW | CORE-ARCH | SB-P-1.12 | IN SCOPE — negative/must-not-appear check, design constraint | Ties directly to `21-§21-1` ("no continuous employee surveillance through permissions tooling"), Contract 21's own already-approved Privacy/Dignity obligation; this mission's Permission Engine must not create any hook enabling continuous location surveillance, independent of whether any location-consuming feature is ever built — no new location-tracking feature implied. **Confirmed unchanged by the Founder's 2026-09-23 dialogue (`founder/03-stage3-founder-product-decision-record.md`); not reopened.** |
| 22-§16-3 | §16 — each feature must define why location is needed (corrected 2026-09-23, MC-16 DRAFT: reconciled per Correction note 7 — see also `12-stage3-gate-evidence-and-row-reconciliation.md` for the general-obligation scope note this cell cannot alone carry) | BUILD NOW | CORE-ARCH | `SB-P-1.18` (named instance: attendance/delivery) | ASSIGNED TO LATER MISSION | **DRAFT — Founder Decision `FPDR-3`: the owning mission of *each* location-consuming feature must itself define and demonstrate this disclosure; `SB-P-1.18` supplies it for attendance/delivery specifically; this is a recurring per-feature obligation that also binds whichever mission later builds any additional location-consuming feature — `SB-P-1.18` is not asserted as the exclusive owner of every future feature's disclosures** | `founder/03-stage3-founder-product-decision-record.md`; `12-stage3-gate-evidence-and-row-reconciliation.md` |
| 22-§16-4 | §16 — each feature must define who may see it (corrected 2026-09-23, MC-16 DRAFT) | BUILD NOW | CORE-ARCH | `SB-P-1.18` (named instance: attendance/delivery) | ASSIGNED TO LATER MISSION | **DRAFT — Founder Decision `FPDR-3`, same basis as `22-§16-3`** | `founder/03-stage3-founder-product-decision-record.md`; `12-stage3-gate-evidence-and-row-reconciliation.md` |
| 22-§16-5 | §16 — each feature must define when it is captured (corrected 2026-09-23, MC-16 DRAFT) | BUILD NOW | CORE-ARCH | `SB-P-1.18` (named instance: attendance/delivery) | ASSIGNED TO LATER MISSION | **DRAFT — Founder Decision `FPDR-3`, same basis as `22-§16-3`** | `founder/03-stage3-founder-product-decision-record.md`; `12-stage3-gate-evidence-and-row-reconciliation.md` |
| 22-§16-6 | §16 — each feature must define how long it is retained (corrected 2026-09-23, MC-16 DRAFT) | BUILD NOW | CORE-ARCH | `SB-P-1.18` (named instance: attendance/delivery) | ASSIGNED TO LATER MISSION | **DRAFT — Founder Decision `FPDR-3`, same basis as `22-§16-3`** | `founder/03-stage3-founder-product-decision-record.md`; `12-stage3-gate-evidence-and-row-reconciliation.md` |
| 22-§16-7 | §16 — each feature must define when access ends (corrected 2026-09-23, MC-16 DRAFT) | BUILD NOW | CORE-ARCH | `SB-P-1.18` (named instance: attendance/delivery) | ASSIGNED TO LATER MISSION | **DRAFT — Founder Decision `FPDR-3`, same basis as `22-§16-3`** | `founder/03-stage3-founder-product-decision-record.md`; `12-stage3-gate-evidence-and-row-reconciliation.md` |
...
**Re-verified 2026-09-23 (MC-16, seventh correction round — DRAFT, pending Mission Control review and Founder human merge).** Following the Founder's confirmed 2026-09-23 dialogue (`mission-control/05-stage3-decision-record-preparation-authorization.md`, `founder/03-stage3-founder-product-decision-record.md`), all 15 `ESCALATED` rows are surgically reconciled to `ASSIGNED TO LATER MISSION`: the 9 Notification rows (`22-§12-1`–`-9`) to `SB-P-1.15`; the 6 Location rows (`22-§16-1`, `22-§16-3`–`-7`) to `SB-P-1.18`. No row added or removed; no source pointer, build commitment or commercial classification changed; `22-§16-2` and `22-§29-9` unchanged. A self-introduced false-positive was caught and fixed before finalizing this round: three rows' Source-pointer cells originally restated the literal word "ESCALATED" in their own correction-note prose, which the disposition-counting `grep` below matched even though those rows' actual Disposition column already read `ASSIGNED TO LATER MISSION` — reworded to remove the bare disposition-name string from non-disposition cells (the same defect class this file's own methodology has caught before), re-verified at zero overlap and an exact 373 total after the fix. Total row count **unchanged at 373** — this round is a disposition/assigned-mission relabeling only, not a re-enumeration.

Reproducible commands re-run against this file at its current head (exact SHA reported in the accompanying gate-evidence file):
...
grep -E "^\| (21|22|20|17|7|BP)-§" 03-stage2-populated-fctm.md | grep -c "IN SCOPE"                        # 228 (unchanged)
grep -E "^\| (21|22|20|17|7|BP)-§" 03-stage2-populated-fctm.md | grep -c "ASSIGNED TO LATER MISSION"       # 113 (was 98, +15 this round)
grep -E "^\| (21|22|20|17|7|BP)-§" 03-stage2-populated-fctm.md | grep -c "DELEGATED"                        # 2 (unchanged)
grep -E "^\| (21|22|20|17|7|BP)-§" 03-stage2-populated-fctm.md | grep -c "NOT APPLICABLE"                    # 30 (unchanged)
grep -E "^\| (21|22|20|17|7|BP)-§" 03-stage2-populated-fctm.md | grep -c "ESCALATED"                          # 0 (was 15, all relabeled this round)
...
grep -E "^\| (21|22|20|17|7|BP)-§" 03-stage2-populated-fctm.md | grep "IN SCOPE" | grep -c "ASSIGNED TO LATER MISSION"  # 0 (no double-counted row)
```

| Contract | Rows | Change this cycle |
|---|---|---|
| 21 | 107 | 0 |
| 22 | 112 | 0 rows added/removed — 15 rows' disposition changed `ESCALATED` → `ASSIGNED TO LATER MISSION` (DRAFT, MC-16) |
| 20 | 47 | 0 |
| 17 | 53 | 0 |
| 7 (limited) | 36 | 0 |
| Build Plan §10.1 + §7 | 18 | 0 |
| **Total** | **373** | **0 (disposition relabel only)** |

**Disposition totals (current DRAFT, MC-16, pending Mission Control review and Founder human merge): `IN SCOPE` 228 + `ASSIGNED TO LATER MISSION` 113 + `DELEGATED` 2 + `NOT APPLICABLE` 30 + `ESCALATED` 0 = 373.** Verified no row double-counts across any pair of dispositions (zero rows match two disposition names in the same cell — re-verified after fixing the false-positive noted above). `UNRESOLVED FOUNDER DECISION`: 0 rows. **Last canonically-merged totals (PR #624, MC-12): `IN SCOPE` 228, `ASSIGNED TO LATER MISSION` 98, `DELEGATED` 2, `NOT APPLICABLE` 30, `ESCALATED` 15 — these remain the canonical record of Product Truth coverage until this pull request is reviewed and human-merged.**

**Citation-gap count: 0 empty cells, 0 bare-implementation-tag-only cells** (unchanged; all 15 reconciled rows carry real, checkable citations to the Founder Product Decision Record).

This total (373) is **not** a target figure — this round changes no row count, only 15 rows' disposition and assigned-mission values, per the Founder's confirmed decision. Every changed row is individually marked with a "corrected 2026-09-23, MC-16 DRAFT" note in its own Source pointer cell.
