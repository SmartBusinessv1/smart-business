...
**Disposition vocabulary:** `IN SCOPE`, `ASSIGNED TO LATER MISSION` (names the mission and citation), `DELEGATED`, `NOT APPLICABLE`, `UNRESOLVED FOUNDER DECISION` (corrected 2026-09-22, MC-10 — first used this round, for Contract 22 §12/§16 items with no Build Plan naming source and no genuine tie to this mission's own already-approved scope; see `06-stage2-delta-evidence.md` Part 4 §3 for the full reasoning and the T1/T3 framing). No row is `ESCALATED` at the row level; the mission-level Stage 3 trigger status is addressed separately in `06-stage2-delta-evidence.md` Part 4 and is **not** pre-cleared by any row here (Mission Control finding F3) — a row-level `UNRESOLVED FOUNDER DECISION` disposition is not itself a Stage 3 gate crossing or a Founder decision; it is a truthful record that no approved source resolves ownership, left for Mission Control.
...
| 22-§12-1 | §12 Notification Foundation — recipient identity/role (corrected 2026-09-22, MC-10: unbundled from a single whole-section placeholder row, which is not permitted for a mixed-disposition section under Source 18 §3.2 item 2) | BUILD NOW | CORE-ARCH | UNRESOLVED | **UNRESOLVED FOUNDER DECISION — no Build Plan §9–12 naming source; not independently testable without a notification-sending feature this mission does not build; not silently absorbed as SB-P-1.12 work** | `06-stage2-delta-evidence.md` Part 4 §3 |
| 22-§12-2 | §12 — language | BUILD NOW | CORE-ARCH | UNRESOLVED | **UNRESOLVED FOUNDER DECISION — same basis as `22-§12-1`; also overlaps Contract 22 §8 Human Language Foundation, already `ASSIGNED` to `SB-P-1.13` (`22-§8`), so any notification-specific instance follows whichever mission actually builds notification delivery, not `SB-P-1.13` by default either** | `06-stage2-delta-evidence.md` Part 4 §3 |
| 22-§12-3 | §12 — channel preference/availability | BUILD NOW | CORE-ARCH | UNRESOLVED | **UNRESOLVED FOUNDER DECISION — same basis as `22-§12-1`** | `06-stage2-delta-evidence.md` Part 4 §3 |
| 22-§12-4 | §12 — template/provider requirements | BUILD NOW | CORE-ARCH | UNRESOLVED | **UNRESOLVED FOUNDER DECISION — same basis as `22-§12-1`** | `06-stage2-delta-evidence.md` Part 4 §3 |
| 22-§12-5 | §12 — delivery/retry state | BUILD NOW | CORE-ARCH | UNRESOLVED | **UNRESOLVED FOUNDER DECISION — same basis as `22-§12-1`** | `06-stage2-delta-evidence.md` Part 4 §3 |
| 22-§12-6 | §12 — duplicate suppression | BUILD NOW | CORE-ARCH | UNRESOLVED | **UNRESOLVED FOUNDER DECISION — same basis as `22-§12-1`** | `06-stage2-delta-evidence.md` Part 4 §3 |
| 22-§12-7 | §12 — privacy | BUILD NOW | CORE-ARCH | UNRESOLVED | **UNRESOLVED FOUNDER DECISION — same basis as `22-§12-1`** | `06-stage2-delta-evidence.md` Part 4 §3 |
| 22-§12-8 | §12 — link to originating business event | BUILD NOW | CORE-ARCH | UNRESOLVED | **UNRESOLVED FOUNDER DECISION — same basis as `22-§12-1`** | `06-stage2-delta-evidence.md` Part 4 §3 |
| 22-§12-9 | §12 — notification delivery separate from business-event completion | BUILD NOW | CORE-ARCH | UNRESOLVED | **UNRESOLVED FOUNDER DECISION — same basis as `22-§12-1`** | `06-stage2-delta-evidence.md` Part 4 §3 |
...
| 22-§16-1 | §16 Location Foundation — shared purpose-limited primitive (use one primitive rather than independent tracking systems) (corrected 2026-09-22, MC-10: unbundled from a single whole-section placeholder row) | BUILD NOW | CORE-ARCH | UNRESOLVED | **UNRESOLVED FOUNDER DECISION — no Build Plan §9–12 naming source for the foundation itself; likely consumers are attendance/delivery under `SB-P-1.18` (see `22-§29-9`), but that does not name an owner for the shared primitive's own construction; not silently absorbed as SB-P-1.12 work** | `06-stage2-delta-evidence.md` Part 4 §3 |
...
| 22-§16-3 | §16 — each feature must define why location is needed | BUILD NOW | CORE-ARCH | UNRESOLVED | **UNRESOLVED FOUNDER DECISION — a disclosure obligation imposed on whichever mission builds the first location-consuming feature; no Build Plan naming source; not this mission's own build** | `06-stage2-delta-evidence.md` Part 4 §3 |
| 22-§16-4 | §16 — each feature must define who may see it | BUILD NOW | CORE-ARCH | UNRESOLVED | **UNRESOLVED FOUNDER DECISION — same basis as `22-§16-3`** | `06-stage2-delta-evidence.md` Part 4 §3 |
| 22-§16-5 | §16 — each feature must define when it is captured | BUILD NOW | CORE-ARCH | UNRESOLVED | **UNRESOLVED FOUNDER DECISION — same basis as `22-§16-3`** | `06-stage2-delta-evidence.md` Part 4 §3 |
| 22-§16-6 | §16 — each feature must define how long it is retained | BUILD NOW | CORE-ARCH | UNRESOLVED | **UNRESOLVED FOUNDER DECISION — same basis as `22-§16-3`** | `06-stage2-delta-evidence.md` Part 4 §3 |
| 22-§16-7 | §16 — each feature must define when access ends | BUILD NOW | CORE-ARCH | UNRESOLVED | **UNRESOLVED FOUNDER DECISION — same basis as `22-§16-3`** | `06-stage2-delta-evidence.md` Part 4 §3 |
...
grep -E "^\| (21|22|20|17|7|BP)-§" 03-stage2-populated-fctm.md | grep -c "UNRESOLVED FOUNDER DECISION"       # 15
...
|---|---|---|
| 21 | 107 | 0 |
| 22 | 112 | +14 (§12 exploded 1→9, §16 exploded 1→7) |
| 20 | 47 | 0 |
| 17 | 53 | 0 |
| 7 (limited) | 36 | 0 |
...
**Disposition totals: `IN SCOPE` 228 + `ASSIGNED TO LATER MISSION` 98 + `DELEGATED` 2 + `NOT APPLICABLE` 30 + `UNRESOLVED FOUNDER DECISION` 15 = 373.** Verified no row double-counts across any pair of dispositions (zero rows match two disposition names in the same cell — this check caught a self-introduced instance of the same class of bug found in rounds 2 and 4: an explanatory note on `22-§29-9` referenced its sibling row's disposition by name, which this file's own `grep` overlap check matched against `22-§29-9`'s own `ASSIGNED TO LATER MISSION` cell; reworded and re-verified at zero overlap before finalizing).

**Citation-gap count: 0 empty cells, 0 bare-implementation-tag-only cells** (both closed in round 3, re-verified unchanged after this round's row additions).
...
**No row is `ESCALATED` at the row level.** 15 rows (`22-§12-1`–`-9`, `22-§16-1`, `22-§16-3`–`-7`) carry `UNRESOLVED FOUNDER DECISION` (corrected 2026-09-22, MC-10 — this document's first use of the value, replacing the two prior whole-section `IN SCOPE (default; unresolved)` placeholder rows, which Source 18 §3.2 item 2 does not permit for a mixed-disposition section) — see `06-stage2-delta-evidence.md` Part 4 §3 for the full reasoning and its T1/T3-candidate framing. A row-level `UNRESOLVED FOUNDER DECISION` disposition is a truthful record that no approved source resolves ownership; it is not itself a Stage 3 gate crossing or a Founder decision, and does not pre-empt Mission Control's own determination. **The mission-level Stage 3 trigger determination is not concluded by this document or by Claude Code** — see `06-stage2-delta-evidence.md` Part 4 §1 for the corrected T1–T8 screen, which leaves T4 and T6 open for Mission Control.
