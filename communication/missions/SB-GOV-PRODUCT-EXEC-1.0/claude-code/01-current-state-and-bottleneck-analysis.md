...
`[L]` `GET /branches/main/protection` on 2026-09-19 returned one required status check (`Markdown Quality Gate`, `strict: true`), `required_approving_review_count: 0`, `enforce_admins: true`, `required_conversation_resolution: true`, `allow_force_pushes: false`. `SB-OPS-CI-ARCHITECTURE-1.0` closed accepted on 2026-09-16 (PR #581, merge `8a6f0df`); the automated baseline is 28 files / 169 tests (8 fast / 61 tests; 20 full / 108 tests). All AWS workflows are `workflow_dispatch` only, so a merge to `main` triggers no deployment.
...
| C-09 | `docs/migration/README.md` lines 55, 85 | "Twelve" / "12" SQL files; 24 exist | `[R]` | Medium |
...
|---|---|---|---|
| O-01 | Fast Gate is not a required status check. `SB-P-1.12`'s standing engineering-quality gate (Build Plan §5.2) is therefore satisfied by a check that does not block merge. | `[L]` | Founder decision; separate branch-protection mission |
| O-02 | `required_approving_review_count` is 0. Human merge is a governance rule enforced by policy and credential separation, not by GitHub review rules. The credential available to this session could read protection settings; merge capability was not tested and no write call was made. | `[L]` | Founder decision; out of scope here |
