...
> **Authority notice:** This v1.2 amendment implements the Founder decisions recorded on PR #605 (comments `5740893113`, `5740990250`, `5741014275` and `5742411779`) and Mission Control's amendment-scope review (comment `5742307415`). It has been independently verified, reviewed by Mission Control and merged by a human in PR #606. **It is not active until Mission Control records the Activation Confirmation defined below.** Until that record exists on canonical `main`, no Product Mission may rely on the amended clauses, and the operative text is Source 18 v1.1 as it stood at commit `c3ef55fe0cf94f4491cb2ae257b084f90b49b40b`. From that record, v1.2 is authoritative for all `SB-P-*` mission lifecycle and delivery work. It remains subordinate to the Lighthouse Constitution, Smart Business constitutional authority, and the approved governance order.
>
> **Activation boundary and post-merge step.** Human merge of this amendment does not activate it. Activation is a distinct Mission Control step after the merge, recorded separately, and it is the only event after which any amended clause is operative. That step (1) records the activation authority; (2) sets Status, Approved By, Approval Date, Activated By, Activation Date and the merge commit in this header and in the Source Change Log, using values that exist only at that time and that this amendment does not pre-write; (3) applies the same reconciliation to every other amended file that carries a pending status: the Elaboration and the Implementation and Evidence templates, the AI Communication and Handover Protocol and the Independent Verification Efficiency Protocol; (4) converts the pending and conditional wording in `AGENTS.md`, `communication/README.md` and the Canonical Source Set into unconditional text; (5) re-verifies the Source Set register row for this file against `main`; and (6) records the Project HQ synchronization status. Until that step is complete, the operative texts are Source 18 Version 1.1, the Elaboration template Version 1.3, the Implementation and Evidence template Version 1.1, the Communication and Handover Protocol Version 1.0 and the Independent Verification Efficiency Protocol Version 1.0, each as it stood at commit `c3ef55fe0cf94f4491cb2ae257b084f90b49b40b`.
>
> **Activation Confirmation.** The Mission Control record that completes that step is the Activation Confirmation. It is entered in Section 8 of the [activation record](../../communication/missions/SB-GOV-PRODUCT-EXEC-1.0/mission-control/04_activation_decision_and_metadata_reconciliation.md) on canonical `main`, after that record's pull request has been independently verified, reviewed by Mission Control and merged by a human, and after Mission Control has verified the actual merged `main`. It activates the five amended instruments together and none of them separately. The merge of PR #606, the opening, verification, review or merge of the activation-record pull request, and any statement of intent do not activate any clause. Only the Activation Confirmation does. The activation-record pull request prepares steps (1) to (6) and records which fields the Activation Confirmation must still set.
